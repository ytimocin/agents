...
- [thesportsdb](thesportsdb/) — TheSportsDB crowdsourced multi-sport database (V1 legacy PHP free with public test key, V2 modern REST behind Patreon paid tier, rich media/artwork CDN, livescore feed for the five tracked leagues, `idAPIfootball` cross-referencing, `{events: null}` transient-sync gotcha, unreliable `strStatus`)
